...
let display = document.querySelector('#display_content');
...
document.addEventListener('fullscreenchange', () => {
  ...
  var dis_quality_btn = document.querySelector("#dis_quality"); 
  ...
  if (document.fullscreenElement === display) {
    ...
    if (video) {
      ...
      dis_quality_btn.style.display = "none"
    ...
    if (video) {
      ...
      dis_quality_btn.style.display = "flex"
...
function add_display_wallpaper() {
  ...
  let wallpaper_image = originaljson[present_wallpaper_code]["link"][display_quality][1];
  let wallpaper_name = originaljson[present_wallpaper_code]["name"];
  // let wallpaper_thumb = originaljson[present_wallpaper_code]["thumbnail"];
  let wallpaper_thumb = originaljson[present_wallpaper_code]["Quality_thumbnail"];
  let wallpaper_type = originaljson[present_wallpaper_code]["type"];
  console.log(wallpaper_name);
  ...
  if (wallpaper_type == "1" || wallpaper_type == "P1") {
    ...
    document.getElementById("display_content").innerHTML = `<div class="loader-container" id="display_loader"><div class="loader"></div></div><video loop  muted src="` + wallpaper_image + `" poster="` + wallpaper_thumb + `" id="view_display">`;
    document.getElementById("display_content").innerHTML += `<div id="play_control" onclick="play_display_video()">
...
`;
 ...
 let view_display = document.getElementById("view_display");

  if (view_display) {
    view_display.addEventListener("progress", updateLoadingBanner);
  


  document.getElementById("display_loader").style.display = "flex";

  function updateLoadingBanner() {
    const buffered = view_display.buffered;
    if (buffered.length > 0) {
      document.getElementById("display_loader").style.display = "none";
      const loadedPercentage = (buffered.end(0) / view_display.duration) * 100;
